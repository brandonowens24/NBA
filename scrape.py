...
def main():
    conn = pymysql.connect(host='nba-v2.cdfzn0kcmufv.us-east-2.rds.amazonaws.com',
                             user='brandon',
    ...
    # REF STATS FOR LAST YEAR AND THIS YEAR
    ...
    # TODAY'S GAMES, TODAY'S REFS, ACCUMULATE REF STATS, ACCUMULATE INJURY STATS, INSERT INTO TABLE
        ...
        def update_table(self):
            ...
            if home_teams and away_teams:
                for home_team, away_team in zip(home_teams, away_teams):
                    ...
                    self.cursor.execute('''SELECT * FROM player_stats WHERE team_tag LIKE (SELECT SUBSTRING(abbreviation, 1, 3) FROM teams WHERE team_name LIKE %s) AND EXISTS (SELECT * FROM injury_report WHERE player_name LIKE CONCAT('%%', player_stats.player_name, '%%'))''', ('% ' + home_team + '%',))
                    home_team_injuries = self.cursor.fetchall()
                    self.cursor.execute('''SELECT * FROM player_stats WHERE team_tag LIKE (SELECT SUBSTRING(abbreviation, 1, 3) FROM teams WHERE team_name LIKE %s) AND EXISTS (SELECT * FROM injury_report WHERE player_name LIKE CONCAT('%%', player_stats.player_name, '%%'))''', ('% ' + away_team + '%',))
    ...
    today_utc = datetime.datetime.now(timezone('EST'))
    yesterday_utc = today_utc - timedelta(days=1)
    date = today_utc.strftime("%Y%m%d") + '0'
    yesterday_date = yesterday_utc.strftime("%Y%m%d") + '0'
